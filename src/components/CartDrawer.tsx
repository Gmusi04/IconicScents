      ...
      {isOpen && (
        <div className="fixed inset-0 z-[110] flex justify-end">
          ...
          <motion.div
            ...
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
            transition={{ duration: 0.45, ease: DRAWER_EASE }}
            className="relative flex h-full w-full max-w-sm flex-col bg-[var(--color-cream)] shadow-2xl"
          >
            <div className="flex items-center justify-between border-b border-[var(--color-line)] px-5 py-4">
              ...
              <button
                aria-label="Cerrar"
                onClick={closeCart}
                className="text-2xl leading-none text-[var(--color-ink)]/60 transition duration-150 ease-out active:scale-90"
              >
                &times;
              </button>
            </div>
              ...
              {items.length === 0 ? (
                ...
                <ul className="flex flex-col gap-4">
                  {items.map((item) => (
                    <li
                      key={item.slug}
                      className="flex items-start justify-between gap-3 border-b border-[var(--color-line)] pb-4"
                    >
                      ...
                      <div className="flex shrink-0 flex-col items-end gap-2">
                        <div className="flex items-center gap-2 rounded-full border border-[var(--color-ink)]/15">
                          <button
                            ...
                            className="flex h-7 w-7 items-center justify-center text-[var(--color-ink)] transition duration-150 ease-out active:scale-90"
                          >
                            −
                          </button>
            ...
            {items.length > 0 && (
              ...
              >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex w-full items-center justify-center rounded-full bg-[var(--color-gold)] px-6 py-3 text-sm font-medium text-[var(--color-ink)] transition duration-150 ease-out active:scale-[0.97] hover:bg-[var(--color-gold-dark)]"
                >
                  Pedir por WhatsApp
                </a>
                <button
